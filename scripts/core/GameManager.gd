...
func _ready():
	...
	# Connect to main menu signals
	connect_to_main_menu()
...
func connect_to_main_menu():
	"""Connect to main menu signals"""
	# Wait a frame to ensure all nodes are ready
	await get_tree().process_frame
	
	# Find the main menu node
	var main_menu = get_tree().get_first_node_in_group("main_menu")
	if not main_menu:
		# Try to find it by name
		main_menu = get_node_or_null("/root/Main/MainMenu")
	
	if main_menu:
		# Connect to main menu signals
		if main_menu.has_signal("new_game_started"):
			main_menu.new_game_started.connect(_on_new_game_started)
		if main_menu.has_signal("game_loaded"):
			main_menu.game_loaded.connect(_on_game_loaded)
		if main_menu.has_signal("settings_opened"):
			main_menu.settings_opened.connect(_on_settings_opened)
		if main_menu.has_signal("quit_game"):
			main_menu.quit_game.connect(_on_quit_game)
		print("GameManager: Connected to main menu signals")
	else:
		print("GameManager: Warning - Main menu not found")

func _on_new_game_started():
	"""Handle new game started signal"""
	print("GameManager: New game started from main menu")
	start_new_game()

func _on_game_loaded(slot_number: int):
	"""Handle game loaded signal"""
	print("GameManager: Loading game from slot %d" % slot_number)
	var save_system = get_node("/root/SaveSystem")
	if save_system:
...
func _on_settings_opened():
	"""Handle settings opened signal"""
	print("GameManager: Settings opened from main menu")
	# TODO: Implement settings panel

func _on_quit_game():
	"""Handle quit game signal"""
	print("GameManager: Quit game requested from main menu")
	get_tree().quit() 
