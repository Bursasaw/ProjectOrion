...
# Version 0.12.0 - Enhanced with encryption, validation, and recovery

signal save_completed(save_name: String)
signal load_completed(save_name: String)
signal save_failed(error: String)
signal load_failed(error: String)
signal save_backup_created(slot_number: int)
signal save_corruption_detected(slot_number: int, error: String)
signal save_recovery_attempted(slot_number: int, success: bool)
...
const MAX_SAVE_SIZE = 10 * 1024 * 1024  # 10MB limit
const BACKUP_INTERVAL = 5  # Create backup every 5 saves
...
# Encryption system
var save_encryption: SaveEncryption
...
func save_game(slot_number: int, save_name: String = "") -> bool:
	...
	# Validate save data before encryption
	if not _validate_save_data(save_data):
		save_failed.emit("Save data validation failed")
		print("Failed to save game - validation failed")
		return false
	
	# Check save file size limit
	var json_string = JSON.stringify(save_data)
	if json_string.length() > MAX_SAVE_SIZE:
		save_failed.emit("Save file too large")
		print("Failed to save game - file size exceeds limit")
		return false
	
	# Encrypt save data
	var encrypted_data = save_encryption.encrypt_save_data(save_data)
	if encrypted_data.is_empty():
		save_failed.emit("Encryption failed")
		print("Failed to save game - encryption failed")
		return false
	...
	if file:
		...
		file.close()
		print("Game saved successfully to slot %d (encrypted)" % slot_number)
		
		# Create backup if needed
		if save_count % BACKUP_INTERVAL == 0:
			_create_save_backup(slot_number)
...
func load_game(slot_number: int) -> bool:
	...
	if loaded_data.is_empty():
		...
		if file:
			...
			if save_encryption == null:
				# Initialize encryption if not already done
				save_encryption = SaveEncryption.new()
			
			loaded_data = save_encryption.decrypt_save_data(encrypted_string)
			
			if loaded_data.is_empty():
				# Try recovery from backup
				if _attempt_save_recovery(slot_number):
				...
				if loaded_data.is_empty():
					load_failed.emit("Failed to decrypt save file")
					save_corruption_detected.emit(slot_number, "Decryption failed")
					corruption_count += 1
					_save_save_statistics()
					return false
		else:
...
func get_save_info(slot_number: int) -> Dictionary:
	"""Get information about a save slot (handles encrypted saves)"""
	var save_path = SAVE_DIR + "slot_" + str(slot_number) + SAVE_EXTENSION
	...
	if file:
		...
		var loaded_save_data = save_encryption.decrypt_save_data(encrypted_string)
		...
		if not loaded_save_data.is_empty():
			return {
				...
				"encrypted": loaded_save_data.get("encrypted", true),
				"compressed": loaded_save_data.get("compressed", false)
			}
		else:
			# Try to get basic info from backup
			var backup_path = SAVE_DIR + "slot_" + str(slot_number) + BACKUP_EXTENSION
...
func get_save_data(slot_number: int) -> Dictionary:
	"""Get the actual save data for a slot (handles encrypted saves)"""
	var save_path = SAVE_DIR + "slot_" + str(slot_number) + SAVE_EXTENSION
	...
	if file:
		...
		print("SaveSystem: Attempting to decrypt save data for slot ", slot_number)
		var decrypted_data = save_encryption.decrypt_save_data(encrypted_string)
		
		if not decrypted_data.is_empty():
			print("SaveSystem: Successfully decrypted save data")
			return decrypted_data
		else:
			print("SaveSystem: Decryption failed, attempting recovery from backup")
			# Try recovery from backup
			if _attempt_save_recovery(slot_number):
				file = FileAccess.open(save_path, FileAccess.READ)
				if file:
					...
					var recovered_data = save_encryption.decrypt_save_data(encrypted_string)
					if not recovered_data.is_empty():
						print("SaveSystem: Successfully recovered save data from backup")
						return recovered_data
					else:
						print("SaveSystem: Recovery from backup also failed")
			else:
				print("SaveSystem: No backup available for recovery")
...
func _validate_save_data(data: Dictionary) -> bool:
	"""Validate that save data has required fields and proper structure"""
	var required_fields = ["version", "timestamp", "player_data", "world_state"]
	
	for field in required_fields:
		if not data.has(field):
			print("SaveSystem: Missing required field: " + field)
			return false
	
	# Validate version compatibility
	var save_version = data.get("version", "")
	if not _is_version_compatible(save_version):
		print("SaveSystem: Incompatible save version: " + save_version)
		return false
	
	# Validate data types
	if not data["player_data"] is Dictionary:
		print("SaveSystem: Invalid player_data type")
		return false
	
	if not data["world_state"] is Dictionary:
		print("SaveSystem: Invalid world_state type")
		return false
	
	return true

func _is_version_compatible(_save_version: String) -> bool:
...
func _create_save_backup(slot_number: int):
	...
	if FileAccess.file_exists(save_path):
		...
		if dir:
			...
			if result == OK:
				print("SaveSystem: Backup created for slot %d" % slot_number)
				backup_count += 1
				save_backup_created.emit(slot_number)
			else:
				print("SaveSystem: Failed to create backup for slot %d" % slot_number)

func _attempt_save_recovery(slot_number: int) -> bool:
	"""Attempt to recover a corrupted save from backup"""
	var backup_path = SAVE_DIR + "slot_" + str(slot_number) + BACKUP_EXTENSION
	var save_path = SAVE_DIR + "slot_" + str(slot_number) + SAVE_EXTENSION
	...
	if FileAccess.file_exists(backup_path):
		var dir = DirAccess.open(SAVE_DIR)
		if dir:
			var result = dir.copy(backup_path, save_path)
			if result == OK:
				print("SaveSystem: Save recovered from backup for slot %d" % slot_number)
				recovery_attempts += 1
				save_recovery_attempted.emit(slot_number, true)
				return true
			else:
				print("SaveSystem: Failed to recover save from backup for slot %d" % slot_number)
				save_recovery_attempted.emit(slot_number, false)
...
func get_save_statistics() -> Dictionary:
	...
	return {
		...
		"corruption_count": corruption_count,
		"recovery_attempts": recovery_attempts,
		"encryption_info": save_encryption.get_encryption_info()
	}
...
func validate_save_file(slot_number: int) -> bool:
	...
	if file:
		...
		return save_encryption.validate_encrypted_data(encrypted_string)
...
func create_manual_backup(slot_number: int) -> bool:
	...
	var save_path = SAVE_DIR + "slot_" + str(slot_number) + SAVE_EXTENSION
	var backup_path = SAVE_DIR + "slot_" + str(slot_number) + BACKUP_EXTENSION + "_manual"
...
func migrate_save_version(slot_number: int) -> bool:
	...
	var migrated_data = get_save_data(slot_number)
	if migrated_data.is_empty():
		return false
	
	# Update version and add new fields
	migrated_data["version"] = SAVE_VERSION
	migrated_data["encrypted"] = true
	migrated_data["compressed"] = false
	
	# Save the migrated data
	return save_game(slot_number, migrated_data.get("save_name", "Migrated Save"))

func export_save_data(slot_number: int, export_path: String) -> bool:
	"""Export save data to a specified path"""
	var export_data = get_save_data(slot_number)
...
func import_save_data(import_path: String, slot_number: int) -> bool:
	...
	if file:
		...
		if parse_result == OK:
			...
			if _validate_save_data(imported_data):
