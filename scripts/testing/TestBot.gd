...
func run_save_load_system_tests():
	...
	await run_test("SaveEncryption_Crypto_API", test_save_encryption_crypto_api)
...
func test_save_encryption_crypto_api() -> Dictionary:
	"""Test SaveEncryption crypto API compatibility and functionality"""
	var result = {"success": false, "details": "", "errors": []}
	
	# Create SaveEncryption instance
	var save_encryption = SaveEncryption.new()
	if not save_encryption:
		result.errors.append("Failed to create SaveEncryption instance")
		return result
	
	# Test if encryption key is properly initialized
	if not save_encryption.test_encryption_key():
		result.errors.append("Encryption key not properly initialized")
		return result
	...
	encrypted_string = save_encryption.encrypt_save_data(test_data)
	if encrypted_string.is_empty():
		result.errors.append("Encryption failed - empty result")
		return result
	
	# Test decryption
	var decrypted_data = {}
	decrypted_data = save_encryption.decrypt_save_data(encrypted_string)
	if decrypted_data.is_empty():
		result.errors.append("Decryption failed - empty result")
		return result
	
	# Verify data integrity
	if not decrypted_data.has("test_key") or decrypted_data["test_key"] != "test_value":
		result.errors.append("Data integrity check failed - test_key mismatch")
		return result
	
	if not decrypted_data.has("number") or decrypted_data["number"] != 42:
		result.errors.append("Data integrity check failed - number mismatch")
		return result
	
	if not decrypted_data.has("array") or decrypted_data["array"].size() != 5:
		result.errors.append("Data integrity check failed - array mismatch")
		return result
	
	if not decrypted_data.has("nested") or not decrypted_data["nested"].has("key"):
		result.errors.append("Data integrity check failed - nested object mismatch")
		return result
	
	# Test encryption info
	var encryption_info = save_encryption.get_encryption_info()
	if not encryption_info.has("algorithm") or encryption_info["algorithm"] != "AES256":
		result.errors.append("Encryption info missing or incorrect algorithm")
		return result
	
	if not encryption_info.has("keys_exist") or not encryption_info["keys_exist"]:
		result.errors.append("Encryption keys not properly initialized")
		return result
	
	result.success = true
	result.details = "SaveEncryption crypto API functional - encryption/decryption working correctly with 2-parameter signature"
	return result
